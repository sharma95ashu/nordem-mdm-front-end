...
const ShippingModuleReturn = ({
  ...
  const { mutate: apiCourierList, isLoading: loadingCourierList } = useMutation(
    "getCourierDetails",
    () =>
      apiService.getCourierList(recordId.ship_rocket_return_order_id, orderDetails?.customer_id),
    {
  ...
  const { isLoading: loadingOrderDetails } = useQuery(
    "singleOrderDetails",
    () => apiService.getShipRocketSingleOrder(recordId.ship_rocket_return_order_id),
    {
  ...
  const handleAssignAwb = (record) => {
    try {
      let data = {
        shipment_id: recordId.ship_rocket_return_shipment_id,
  ...
  useEffect(() => {
    const find = orderDetails.return_order_items.find(
      (e) => e.depot_order_id === recordId.depot_order_id
    );
    if (find) {
      setRecordData(find);
    }
              ...
              <Row gutter={[24, 16]}>
                <Col xs={12} lg={8}>
                  <Space direction="vertical" size={10}>
                    ...
                    <Typography.Text>
                      {`${singleOrderDetails?.return_pickup_data?.address},
                        `}
                    </Typography.Text>
                    <Typography.Text>
                      {`${singleOrderDetails?.return_pickup_data?.pin_code},
                        ${singleOrderDetails?.return_pickup_data?.state}`}
                    </Typography.Text>
                  </Space>
                </Col>
                <Col xs={12} lg={8}>
...
export default ShippingModuleReturn;

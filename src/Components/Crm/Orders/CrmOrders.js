...
import { CART_TYPE, snackBarErrorConf, snackBarSuccessConf } from "Helpers/ats.constants";
...
import { getDepoOrderColums } from "./Columns";
...
const CrmOrders = () => {
  ...
  const [isNoPendingStatus, setIsNoPendingStatus] = useState(null);
  ...
  const { refetch: getSingleOrder, isLoading: getSingleLoader } = useQuery(
    ...
    () => apiService.getCrmOrdersByID(params.id),
    {
      ...
      onSuccess: (data) => {
        if (data?.success) {
          ...
          if (data?.data?.cart_type === CART_TYPE.DEPOT) {
            // Store the result in state

            const hasPendingItems = data?.data?.order_items?.some(
              (item) => item.status === "pending"
            );

            setIsNoPendingStatus(hasPendingItems);

            const offerItemsMap = data?.data?.order_items.map((e) =>
              e.order_items
            ...
            const output = data?.data?.order_items?.map((e) => {
              let newOrder = e.order_items
                ...
                .map((item, index) => {
                  ...
                  return {
                    ...item,
                    // order_items: item.order_items?.map((item) => ({
                    //   ...item,
            ...
            const offerItemsMap = data?.data?.order_items
              ?.filter((item) => item.is_offer_product && !item.is_only_offer_purchasable)
              .reduce((acc, item) => {
                const offerId = item.offer_id;
                if (!acc[offerId]) {
            ...
            // Map to transform the input into the desired output
            const output = data?.data?.order_items
              .filter((item) => !item.is_offer_product || item.is_only_offer_purchasable)
              .map((item, index) => ({
                ...item,
                total_accepted_qty: item.quantity,
                sr_no: index + 1,
                offer_items: offerItemsMap[item.offer_id] || []
              }));
    ...
    (data) => apiService.createInvoiceDepot(data),
    {
      onSuccess: (data) => {
        if (data?.success && data?.data?.length && data?.data[0].INVOICE) {
          ...
          // let recordData = orderDetails.order_items.filter(
          //   (e) => e.depot_order_id === data?.data[0].CUST_REF
          // );

          // if (orderDetails?.cust_type !== "DEPOT") {
  ...
  const { mutate: shipRocketOrderCreate, isLoading: shippingRocketLoading } = useMutation(
    (data) => apiService.addShipRocketApi(data),
    {
      onSuccess: (data) => {
        if (data?.success) {
          getSingleOrder();
  ...
  // handle reset confirm order and cancel items
  const handleReset = () => {
    setShowButton(false);
    setTableModal(false);
    setCheckField(null);
  ...
  const { mutate: returnRefund, isLoading: returnRefundLoader } = useMutation(
    (data) => apiService.returnOrderItem(data),
    {
      onSuccess: (data) => {
        if (data?.success) {
          setRefundData(data?.data);
  ...
  const handleDepotOrder = (record, orderNo, key) => {
    try {
      ...
      // return;
      if (key && key == "cancel") {
        filter["remark"] = "Cancelled by CRM";
  ...
  const addOrderShipRocket = async (recordData) => {
    try {
      let sum = 0;
      let orderItems = recordData?.order_items?.map((item) => {
      ...
      let shipData = await apiService.getShippingChargesForDepot({
        total: sum,
        customerType: orderDetails?.cust_type,
        dispatch_by: orderDetails.cart_type
      });
      ...
      let cost = 0;
      if (shipData?.success) {
        cost = shipData.data.shipping_charges;
      } else {
        return enqueueSnackbar("Some issue while creating shipping order", snackBarErrorConf);
      }

      let data = {
        order_id: recordData.depot_order_id,
        order_date: new Date().toISOString(),
        pickup_location: "Rcm Consumer",
        // billing_customer_name: orderDetails?.shipping_name,
        billing_customer_name: "NORDEM TEST DON,T PROCESS IT",
        billing_last_name: "DIST02",
        billing_address: orderDetails?.shipping_address,
        billing_city: orderDetails?.initial_address?.city,
        billing_pincode: orderDetails?.initial_address?.pincode,
        billing_state: orderDetails?.initial_address?.state,
        billing_email: orderDetails?.initial_address?.recipient_email,
        billing_phone: orderDetails?.shipping_mobile,
        ...
        total_discount: "0.00",
        sub_total: Number(sum),
        shipping_charges: cost
      };
      // ship rocket api call for create order
      shipRocketOrderCreate(data);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const { mutate: getShipRocketOrderDetails, isLoading: loadingOrderDetails } = useMutation(
    ...
    (recordData) => apiService.getShipRocketSingleOrder(recordData.ship_rocket_order_id),
    {
                        ...
                        {orderDetails?.order_to == CART_TYPE.DEPOT ? (
                          <Col span={24}>
                            <DepotTable
                                        ...
                                        render: (text, record) => {
                                          ...
                                          const items = [
                                            record?.awb != null && {
                                              ...
                                              label: (
                                                <Typography.Text>Download Invoice</Typography.Text>
                                              )
                                            },
                                            ...
                                            record?.ship_rocket_order_id != null &&
                                              record.awb == null && {
                                                key: "add_dimension",
                                                label: (
                                                  <Typography.Text>Add Dimension</Typography.Text>
                                                )
                                              },
                                            ...
                                            record?.awb != null && {
                                              key: "download_label",
                                              label: (
                                                <Typography.Text>Download Label</Typography.Text>
                                              )
                                            },
                                            record?.awb != null && {
                                              key: "order_reassign",
                                              ...
                                              {record.status === CARD_STATUS.CONFIRMED &&
                                                ...
                                                !isNoPendingStatus && (
                                                  <Popconfirm
                                                    ...
                                                    description="Are you sure to Generate Invoice ?"
                                                    onConfirm={() => {
                                              ...
                                              {record.invoice_generated == true &&
                                                ...
                                                record?.ship_rocket_order_id === null && (
                                                  <Button
                                                    ...
                                                    onClick={() => {
                                                      // setRecordData(record);
                                                      addOrderShipRocket(record);
                                                      // setShowShippingScreen(true);
                                                    }}>
                                                    Proceed With ShipRocket
                                                  </Button>
                                                )}
                              ...
                              data={offerTableData}
                              expandedColumnsList={getDepoOrderColums(

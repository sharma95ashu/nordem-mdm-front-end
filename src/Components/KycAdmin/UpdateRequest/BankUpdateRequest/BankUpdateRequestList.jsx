...
const BankUpdateRequestList = () => {
        ...
        <Flex gap="small" justify="center" align="center">
          <>
            <Typography.Text
              ...
              onClick={() => navigate(`/${KycAdminPaths.viewBankUpdateRequest}/${record.dist_no}`)}>
              View
            </Typography.Text>
          </>
        </Flex>
  ...
  // useQuery to fetch bank update requests
  const { mutate: getBankUpdateRequests, isLoading } = useMutation(
    "fetchBankUpdateRequests",
    (data) => apiService.getAllBankUpdateRequests(data),
    {
      enabled: false, // Fetch only when payload is available
      onSuccess: (res) => {
  ...
  const handleSearch = (values) => {
    try {
      ...
      // api call
      current == 1 ? getBankUpdateRequests(data) : setCurrent(1);
  ...
  useEffect(() => {
    ...
    getBankUpdateRequests(data); // api call
  }, [current, pageSize]);
        ...
        <Col span={24}>
          <Flex justify="space-between" vertical gap={8}>
            ...
            <Typography.Text style={StyleSheet.fontSize14} className="removeMargin">
              <Typography.Text type="secondary">Update Request /</Typography.Text> Bank Update
              Request
            </Typography.Text>
          </Flex>
        </Col>
...
export default BankUpdateRequestList;

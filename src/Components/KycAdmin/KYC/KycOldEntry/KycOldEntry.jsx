...
import ApproveRejectSection from "../KycNewEntry/ApproveRejectSection";
...
const KycOldEntry = () => {
  ...
  const [decalarationDataSource, setDecalarationDataSource] = useState([]);
  ...
  // ---------------- Get Kyc New Entities ----------------
  const { isLoading: loadingOldEntities } = useQuery(
    ["getKycOldEntryDetails", payload],
    () => apiService.getKycOldEntryDetails(payload),
    {
      enabled: !!payload,
      // Configuration options for the mutation
      onSuccess: ({ data, success, message }) => {
        if (success) {
          ...
          if (data?.security_details?.length > 0) {
            setDecalarationDataSource(data.security_details);
          }
        }
      },
      onError: (error) => {
  ...
  const { mutate: updateKycStatusMutate, isLoading: KycUpdateLoading } = useMutation(
    (request) => apiService.updateOldKycStatus(request),
    {
      // Update confirmation
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <Spin spinning={loadingOldEntities || KycUpdateLoading || fetchingPhoto} size="large">
      <Row gutter={[20, 24]}>
        <Col span={24}>
          <SearchByComponent
            ref={searchRef}
            ...
            searchLoading={loadingOldEntities}
            moduleName={"kyc_old_entry"}
          />
        </Col>
        {show ? (
              ...
              <Flex className="fullWidth" span={24} vertical gap={12}>
                <>
                  ...
                  {ABDetails?.PERSONAL_INFO?.Declaration?.toLowerCase()?.startsWith("y") &&
                    decalarationDataSource.length > 0 && (
                      <Flex vertical gap={12}>
                        <Text strong style={StyleSheet.sectionHeading}>
                          {"Associate Buyer Decalaration"}
                        </Text>
                        <Flex gap={10}>
                          ...
                          <Table
                            ...
                            dataSource={decalarationDataSource}
                            pagination={false}
                            scroll={{ x: true }}
                          />
                        </Flex>
                ...
                <ApproveRejectSection
                  currentStatus={ABDetails?.kyc_status ? true : false}
          ...
          <SearchByFallbackComponent
            ...
            subTitle={
              "Quickly Search by Associate Buyer Number to process the KYC Entry Old details."
            }
            image={searchByIcon}
          />
        )}

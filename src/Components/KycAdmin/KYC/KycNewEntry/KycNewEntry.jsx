...
const KycNewEntry = () => {
  ...
  const [newKycAB, SetNewKycAB] = useState(null);
  ...
  const handleSearch = (searchValue) => {
    try {
      if (searchValue) {
        // Reset the form before new search
  ...
  const { isLoading: loadingNewEntryDetails } = useQuery(
    ["getKycNewEntryDetails", payload],
    () => apiService.getKycNewEntryDetails(payload),
    {
      enabled: !!payload,
  ...
  const { mutate: updateKycStatusMutate, isLoading: KycUpdateLoading } = useMutation(
    (request) => apiService.updateNewKycStatus(request),
    {
      // Update confirmation
      onSuccess: (response) => {
        if (response?.success) {
          if (response?.data) {
            SetNewKycAB(response?.data);
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <Spin spinning={loadingNewEntryDetails || KycUpdateLoading || fetchingPhoto} size="large">
      <Row gutter={[20, 24]}>
        <Col span={24}>
          <SearchByComponent
            ref={searchRef}
            ...
            searchLoading={loadingNewEntryDetails}
            moduleName={"kyc_new_entry"}
          />
        </Col>
        {show ? (
        ...
        <Modal
          ...
          <Flex vertical gap={16} justify="center" align="center">
            <Typography.Title level={5} className="removeMargin">
              Associate Buyer Number of the checked KYC!
            </Typography.Title>
            <Typography.Title level={5} className="removeMargin ">
              KYC Number -{" "}
              <Typography.Text

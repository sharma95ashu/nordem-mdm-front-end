...
const BranchList = () => {
  ...
  // Function to fetch branches table  data
  const fetchTableData = async (reset = false) => {
    try {
      ...
      let apiUrl = `/branches/get-all`;

      // Make an API call to get the table data
      const data = await apiService.getBranchList(apiUrl, payload);
  ...
  const { data, mutate: refetch, isLoading } = useMutation("fetchAllBranches", fetchTableData);
 ...
 const columns = [
    ...
    render: (text, record) => (
      <Flex gap="small" justify="center" align="center">
        {actionsPermissionValidator(window.location.pathname, PermissionAction.EDIT) && (
          <Button
            type="default"
            onClick={() => navigate(`/${Paths.branchEdit}/${record.branch_id}`)}
          >
            Edit
          </Button>
        )}
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>Manage Branches</Typography.Title>
        ...
        <Col className="gutter-row" span={24}>
          <Flex gap="middle" justify="space-between">
            <Search
              ...
              size="large"
              placeholder="Search by Bank Name or Branch Name"
              value={searchTerm}

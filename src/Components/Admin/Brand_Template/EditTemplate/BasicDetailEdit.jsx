...
export default function BasicDetailEdit({ onSave }) {
  const {
  ...
  const id = pathname.match(/\/edit\/(\d+)$/)?.[1] || null; // Extract ID using regex
        ...
        getBase64(info.fileList[0].originFileObj, (url) => {
          ...
          if (currIndex >= 0) {
            ...
            if (carouselImages?.length == 1) {
              ...
              const fileExtension = `${carouselImages?.length - 1}.${getFileExtension(
                obj.file.name
              )}`;
              const modifiedFile = new File([obj.file], fileExtension, {
                type: obj.file.type,
  ...
  const { mutate: fetchSingleBanner, isLoading: basicLoading } = useMutation(
    (data) => apiService.getKeySoulBasicDetail(data.id),
    {
      ...
      onSuccess: (data) => {
        if (data) {
          if (data.success) {
            if (data?.data?.banner_images?.length > 0) {
            ...
            if (data?.data?.banner_mobile_images?.length > 0) {
              const tempSliderImgs = data?.data?.banner_mobile_images?.map((item) => {
                let obj = { ...item };
              ...
              setCarouselImagesM([...tempSliderImgs, { imgUrl: null }] || []);
            }

            getAllCategorybyBrand(data?.data?.template_type);
            ...
            form.setFieldValue("platform_type", data?.data?.platform_type);
            onSave(data?.data?.brand_id, data?.data?.platform_type); // Trigger the callback to set the brand ID
      ...
      if (carouselImages?.length > 1) {
        ...
        for (let i = 0; i < arr.length; i++) {
          ...
          if (arr[i].active === "undefined") {
            ...
            formData.append("banner_details[" + i + "]" + "active", arr[i].active);
          }
          ...
          if (arr[i].dateRange) {
            ...
            formData.append("banner_details[" + i + "]" + "start_date", arr[i].start_date || null);
            formData.append("banner_details[" + i + "]" + "end_date", arr[i].end_date || null);
          }
        }
      }
      if (carouselImagesM?.length > 1) {
                    ...
                    <Col className="gutter-row" span={12}>
                      <Form.Item
                        ...
                        <Select
                          size="large"
                          disabled
                          options={[{ label: "Keysoul", value: "key-soul" }]}
                          allowClear
                          value={templateType}

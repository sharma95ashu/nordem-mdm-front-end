...
export default function PincodeAreaMappingList() {
  const { Search } = Input;
  ...
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/pincode_area_maps/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  const { mutate: deleteMutate } = useMutation(
    ...
    (data) => apiService.deletePincodeAreaMapping(data.load),
    {
      ...
      onSuccess: (res, payload) => {
        ...
        queryClient.invalidateQueries("fetchPincodeAreaMappingData");
        refetch();
      },
      onError: (error) => {
        setSelectedRowKeys([]);
  ...
  const handleDelete = (data) => {
    try {
      ...
      const body = {
        pinMapIds: Array.isArray(data) ? filterDeleteIds(data) : [data.pincode_area_map_id]
      };
  ...
  const handleEdit = (data) => {
    navigate(`/${Paths.pincodeAreaMappingEdit}/${data.pincode_area_map_id}`);
  };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Pincode Area Maps</Typography.Title>
            ...
            <Flex justify="space-between" gap="middle">
              ...
              <Flex justify="space-between" gap="middle">
                ...
                {actionsPermissionValidator(window.location.pathname, PermissionAction.ADD) && (
                  <NavLink to={`/${Paths.pincodeAreaMappingAdd}`}>

...
export default function DownloadLanguageList() {
  const { Search } = Input;
  ...
  // Function to fetch download language data
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/download_language/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  // Function to delete download language
  const { mutate: deleteMutate } = useMutation(
    (data) => apiService.deleteDownloadLanguage(data.load),
    {
  ...
  // Function to handle deletion of download language
  const handleDelete = (data) => {
    try {
      // Create a request body with downloadLanguageIds array
      const body = {
        downloadLanguageIds: Array.isArray(data) ? filterDeleteIds(data) : [data.language_id]
      };
  ...
  // Function to handle Edit of download language
  const handleEdit = (data) => {
    navigate(`/${Paths.downloadLanguageEdit}/${data.language_id}`);
  };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Download Languages</Typography.Title>
            ...
            <Flex justify="space-between" gap="middle">
              ...
              <Flex justify="space-between" gap="middle">
                ...
                {actionsPermissionValidator(window.location.pathname, PermissionAction.ADD) && (
                  <NavLink to={`/${Paths.downloadLanguageAdd}`}>

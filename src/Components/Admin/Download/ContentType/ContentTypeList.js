...
export default function ContentTypeList() {
  ...
  const columns = [
    {
      ...
      sorter: (a, b) => a.content_type_name.localeCompare(b.content_type_name),
  ...
  // Function to fetch content type data
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/content_type/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  const { mutate: deleteMutate } = useMutation((data) => apiService.deleteContentType(data.load), {
  ...
  const { data, refetch, isLoading } = useQuery("fetchContentTypesData", fetchTableData);

  const filterDeleteIds = (deleteArr) => {
    try {
      const filteredKeys = data
        .filter((obj) => deleteArr.includes(obj.key))
        .map((obj) => obj.content_type_id);
      return filteredKeys;
    } catch (error) {}
  };

  // Function to handle deletion of content type
  const handleDelete = (data) => {
    try {
      // Create a request body with contentTypeIds array
      const body = {
        contentTypeIds: Array.isArray(data) ? filterDeleteIds(data) : [data.content_type_id]
      };

      // Define the request object with the API endpoint and request body
      let obj = { load: body };

      // Perform the API call for category deletion
      deleteMutate(obj);
    } catch (error) {}
  };

  // Function to handle Edit of content type
  const handleEdit = (data) => {
    navigate(`/${Paths.contentTypeEdit}/${data.content_type_id}`);
  };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Content Types</Typography.Title>
            ...
            <Flex justify="space-between" gap="middle">
              ...
              <Flex justify="space-between" gap="middle">
                ...
                {actionsPermissionValidator(window.location.pathname, PermissionAction.ADD) && (
                  <NavLink to={`/${Paths.contentTypeAdd}`}>

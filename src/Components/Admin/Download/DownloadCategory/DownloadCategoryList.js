...
export default function DownloadCategoryList() {
  const { Search } = Input;
  ...
  const columns = [
    {
      ...
      sorter: (a, b) => a.download_category_name.localeCompare(b.download_category_name),
      key: "pincode"
    },
    {
      title: "Display Order",
  ...
  // Function to fetch download category data
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/download_categories/all/${
  ...
  // Function to delete download category
  const { mutate: deleteMutate } = useMutation(
    (data) => apiService.deleteDownloadCategory(data.load),
    {
  ...
  const filterDeleteIds = (deleteArr) => {
    try {
      const filteredKeys = data
        ...
        .map((obj) => obj.download_category_id);
      return filteredKeys;
    } catch (error) {}
  };

  // Function to handle deletion of download category
  const handleDelete = (data) => {
    try {
      // Create a request body with downloadCategoryIds array
      const body = {
        downloadCategoryIds: Array.isArray(data)
          ? filterDeleteIds(data)
  ...
  // Function to handle Edit of download category
  const handleEdit = (data) => {
    navigate(`/${Paths.downloadCategoryEdit}/${data.download_category_id}`);
  };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Download Categories</Typography.Title>
            ...
            <Flex justify="space-between" gap="middle">
              ...
              <Flex justify="space-between" gap="middle">
                ...
                {actionsPermissionValidator(window.location.pathname, PermissionAction.ADD) && (
                  <NavLink to={`/${Paths.downloadCategoryAdd}`}>

...
export default function EventsList() {
  ...
  const columns = [
    {
      ...
      render: (value) => {
        return (
          <Space>
            <Typography.Text>{value ? splitConversion(value).toUpperCase() : ""}</Typography.Text>
          </Space>
        );
      }
    },
    ...
    },
    {
      ...
      key: "start_time",
      render: (value, record) => {
        const startTimeFormatted = getAntTimeFormat(value);
        const endTimeFormatted = getAntTimeFormat(record.end_time);
        return (
          <>
  ...
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/rcm-event/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  const { mutate: deleteMutate } = useMutation((data) => apiService.deleteEvent(data.load), {
    // Configuration options for the mutation
    onSuccess: (res, payload) => {
      if (res) {
        if (data.length == payload?.load?.eventIds?.length) {
          setCurrent(1);
        }
  ...
  const handleDelete = (data) => {
    try {
      ...
      const body = {
        eventIds: Array.isArray(data) ? filterDeleteIds(data) : [data.event_id]
      };
  ...
  const handleEdit = (data) => {
    navigate(`/${Paths.eventsEdit}/${data.event_id}`);
  };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Events List</Typography.Title>

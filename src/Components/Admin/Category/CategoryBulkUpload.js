...
import { snackBarErrorConf, snackBarSuccessConf, categorySheetData } from "Helpers/ats.constants";
...
const CategoryBulkUpload = (props) => {
  ...
  const handlefile = (file) => {
    try {
      console.log("file", file, checkFileExtension(file.name, "xlsx"));
      if (
      ...
      return new Promise(async (resolve, reject) => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (isEmpty(d["Parent"])) {
            ...
            if (!d["Category Name"] || isEmpty(d["Category Name"].toString())) {
              isValid = false;
              d["errors"]["Category Name"] = "Category name cannot be empty";
            }

            if (!d["Display Order"] || isEmpty(d["Display Order"].toString())) {
              isValid = false;
              d["errors"]["Display Order"] = "Display Order cannot be empty";
            }

            // const showOnType = d["Show On Type"]?.toString().split(",");
            // if (
            //   !d["Show On Type"] ||
            //   isEmpty(d["Show On Type"].toString()) ||
            //   !showOnType?.some((type) => ["web", "app"].includes(type))
            // ) {
            //   isValid = false;
            //   d["errors"]["Show On Type"] =
            //     "Show On Type cannot be empty and only web,app or both values are allowed";
            // }

            if (
              !d["Status"] ||
              isEmpty(d["Status"].toString()) ||
              !["active", "inactive"].includes(d["Status"])
            ) {
              isValid = false;
              d["errors"]["Status"] =
                "Category Status cannot be empty and only active/inactive value is allowed";
            }
  ...
  const downloadSheet = () => {
    try {
      const data = categorySheetData.categorySheet;
      ...
      const instructionData = [
        {
          ...
          Description: "This column represents the name of the category."
        },
        {
          "Column Name": "Display Order",
          Description: "This column determines the order in which categories are displayed."
        },
        // {
        //   "Column Name": "Is Featured",
        //   Description: "This column indicates whether an item is featured or not."
        // },
        {
          "Column Name": "Description",
          Description: "This column provides a description of the item."
        },
        {
          "Column Name": "Status",
          Description:
            "This column represents the status of the item whether it is active or inactive."
        },
        // {
        //   "Column Name": "Show On Type",
        //   Description: "This column indicates where the item is shown."
        // },
        // {
        //   "Column Name": "Supported Stores",
        //   Description: "This column lists the stores where the item is supported."
        // },
        {
          "Column Name": "Category Image",
          Description: "This column is for category image link."
        },
        {
          "Column Name": "Banner Image",
          Description: "This column is for banner image link"
        },
        {
          "Column Name": "Banner Mobile Image",
          Description: "This column is for banner image link for mobile"
        }
      ];
      ...
      XLSX.writeFile(workbook, "Category-Sample-Sheet.xlsx");
    } catch (error) {}
  };
  ...
  const { mutate, isLoading } = useMutation(
    ...
    (data) => apiService.uploadCategoryBulkUploadFile(data.load, data.randomId, true),
    {

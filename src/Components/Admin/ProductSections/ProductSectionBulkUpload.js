...
import { snackBarErrorConf } from "Helpers/ats.constants";
import { enqueueSnackbar } from "notistack";
import { isEmpty } from "lodash";
import { TextTruncate } from "Helpers/ats.helper";

const ProductSectionBulkUpload = (props) => {
    ...
    try {
      ...
      if (file) {
        ...
        reader.onload = async (e) => {
          ...
          if (rLength === 0) {
            enqueueSnackbar("No records found in sheet", snackBarErrorConf);
          } else if (rLength > 4000) {
            enqueueSnackbar("Allow only 4000 records at once", snackBarErrorConf);
          } else if (file.size > maxSize) {
            enqueueSnackbar("Allow only max 4MB file", snackBarErrorConf);
          } else {
            setrecordFile(file);
            const columns = await generateColumn(records);

            const expectedColumnTitles = ["SAP Code"];
    ...
    try {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (!d["SAP Code"] || !/^[0-9]\d*$/.test(d["SAP Code"])) {
            isValid = false;
            d["errors"]["SAP Code"] = "Only numeric values are allowed ";
          }
          nodes.push({
            ...d,
            key: i
    ...
    <>
      <Modal
        ...
        footer={
          showType == "show_list"
            ? [
                <Button key="back" onClick={() => handelModel(false)}>
                  Cancel
                </Button>,
                <Button key="submit" type="primary" onClick={sendFile} disabled={validFile}>
...
export default ProductSectionBulkUpload;

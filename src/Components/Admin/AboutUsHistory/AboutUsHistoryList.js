...
// About Us - History List Component
const AboutUsHistoryList = () => {
  const { Search } = Input;
  ...
  const fetchTableData = async (reset = false) => {
    try {
      ...
      let baseUrl = `/about-us-history/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  const { data, mutate: refetch, isLoading } = useMutation("fetchAbtUsHistoryData", fetchTableData);
          ...
          {actionsPermissionValidator(window.location.pathname, PermissionAction.EDIT) && (
            <>
              <Button
                type="default"
                primary
                onClick={() => navigate(`/${Paths.aboutUsHistoryEdit}/${record.history_id}`)}>
                Edit
              </Button>
            </>
          )}
  ...
  // api  to delete about us history
  const { mutate: deleteMutate } = useMutation(
    (data) => apiService.deleteSingleAboutUsHistory(data.load),
    {
  ...
  const filterDeleteIds = (deleteArr) => {
    try {
      const filteredKeys = data
        ...
        .map((obj) => obj.history_id);
      return filteredKeys;
    } catch (error) {}
  };

  // Function to handle deletion of about us - history
  const handleDelete = (data) => {
    try {
      // Create a request body with tagsIds array
      const body = {
        historyIds: Array.isArray(data) ? filterDeleteIds(data) : [data.history_id]
      };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>About Us - History</Typography.Title>
            ...
            <Flex justify="space-between" gap="middle">
              ...
              {actionsPermissionValidator(window.location.pathname, PermissionAction.ADD) && (
                <Flex style={StyleSheet.flexFullStyle} gap="middle" justify="flex-end">
                  <NavLink to={`/${Paths.aboutUsHistoryAdd}`}>
...
export default AboutUsHistoryList;

...
export default function CouponAdd() {
  ...
  const onFinish = (value) => {
    ...
    if (value?.applicable_users && typeof value?.applicable_users[0] == "object") {
      value?.applicable_users?.forEach((item) => {
        tempApplicableUser.push(String(item?.value));
      });
    } else if (value?.applicable_users && value?.applicable_users?.length > 0) {
      tempApplicableUser = value?.applicable_users?.toString().split(",");
    } else {
    ...
    let obj = { load: { ...data, applicable_users: tempApplicableUser } };
  ...
  const { mutate, isLoading } = useMutation(
    ...
    (data) => apiService.updateCoupon(data.load, params?.id),
    {
  ...
  const { refetch } = useQuery(
    ...
    () => apiService.getRequest(`/coupons/${params.id}`),
    {
      // Configuration options
      ...
      onSuccess: (data) => {
        ...
        if (data) {
          ...
          let start_expiry_date = [];

          start_expiry_date.push(dayjs(data?.data?.start_date));
          start_expiry_date.push(dayjs(data?.data?.expiry_date));

          form.setFieldValue("select_start_date_expiry_date", start_expiry_date);

          form.setFieldValue(
            "coupon_status",
            data?.data?.coupon_status === "active" ? true : false
          );
          ...
          let typeIds = [];
          data?.data?.type_ids.map((item) => typeIds.push(Number(item)));
          form.setFieldValue("type_ids", typeIds);

          if (data?.data?.applicable_on === "product") {
            fetchApplicableProduct();
            SetApplicableOn(data?.data?.applicable_on);
            const tempProductList = data?.data?.type_ids?.map((item) => ({
              value: item?.product_id,
              label: item?.product_name
            }));
            SetApplicableOnIds(tempProductList);
            form.setFieldValue(
              "type_ids",
              data?.data?.type_ids?.map((e) => e.product_id)
            );
          } else if (data?.data?.applicable_on === "category") {
            fetchApplicableCategory();
            SetApplicableOn(data?.data?.applicable_on);

            form.setFieldValue(
              "type_ids",
              data?.data?.type_ids?.map((e) => e.category_id)
            );
          } else {
            fetchApplicableBrand();
            SetApplicableOn(data?.data?.applicable_on);
            ...
            form.setFieldValue(
              "type_ids",
              data?.data?.type_ids?.map((e) => e.brand_id)
            );
          }
        }
      },
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.EDIT) ? (
    <>
      <Typography.Title level={5}>Edit Coupon</Typography.Title>
        ...
        <Row gutter={[24, 0]}>
          ...
          <Col
            ...
            <Form.Item
              ...
              rules={[{ required: true, message: "Start date & end date is required" }]}>
              <RangePicker
                  ...
                  rules={
                    disableMinPurchaseAmount
                      ...
                      : [
                        ...
                        { pattern: /^[0-9]\d*$/, message: "Please enter valid number" },
                        {
                          pattern: /^(?:0|[1-9]\d{0,9}(?:\.\d+)?|1000000000(?:\.0+)?)$/,
                          message: "Please enter a number up to 1,000,000,000"
                        },

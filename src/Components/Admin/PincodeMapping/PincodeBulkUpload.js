...
import { snackBarErrorConf, snackBarSuccessConf, pincodeSheetData } from "Helpers/ats.constants";
...
const PincodeBulkUpload = (props) => {
            ...
            return new Promise(async (resolve, reject) => {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (isEmpty(d["Parent"])) {
                        ...
                        let pin_code_formetCheck = no_special_character(d["Pincode"]);
                        let store_code_formetCheck = no_special_character(d["Store Code"]);

                        let pincodeValid = pincodeCheck(d["Pincode"]);
                        let storeCodeValid = storeCodeCheck(d["Store Code"]);

                        // My data error check start
                        if (!d["Pincode"] || isEmpty(d["Pincode"].toString()) || parseInt(d["Pincode"]) <= 0 || !pin_code_formetCheck || !pincodeValid) {
                            isValid = false;
                            d["errors"]["Pincode"] = "Pin Code must be exactly 6 digits long, starting with a non-zero number. Only numeric values are allowed.";
                        }

                        if (!d["Store Code"] || isEmpty(d["Store Code"].toString()) || parseInt(d["Store Code"]) <= 0 || !store_code_formetCheck || !storeCodeValid) {
                            isValid = false;
                            d["errors"]["Store Code"] = "Store Code cannot be empty, zero, or negative. It should contain only numbers and must be at least 3 digits long.";
                        }

                        if (!d["Store Name"] || isEmpty(d["Store Name"].toString())) {
                            isValid = false;
                            d["errors"]["Store Name"] = "Field cannot be empty";
                        }
    ...
    const downloadSheet = () => {
        try {
            const data = pincodeSheetData.pincodeSheet;
            ...
            const instructionData = [
                { 'Column Name': "Store Name", Description: "Enter the correct store name matching the system record." },
                { 'Column Name': "Pin Code", Description: "Enter a valid numeric pin code corresponding to the store's delivery area." },
                { 'Column Name': "Store Code", Description: "Enter a valid store code." },
            ];
            const ws3 = XLSX.utils.json_to_sheet(instructionData);
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, ws, "Pincode List");
            XLSX.utils.book_append_sheet(workbook, ws3, "Instructions");
            XLSX.writeFile(workbook, "Pincode-Sample-Sheet.xlsx");
        } catch (error) { }
    };
    ...
    const { mutate, isLoading } = useMutation(
        ...
        (data) => apiService.uploadPincodeBulkUploadFile(data.load, data.randomId, true),
        {
            ...
            onSuccess: (res, variables) => {
                if (res.success) {
                    ...
                    enqueueSnackbar("Pincodes uploaded successfully", snackBarSuccessConf);
                } else {
...
export default PincodeBulkUpload;

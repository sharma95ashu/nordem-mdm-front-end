...
import { Button, Col, Flex, Form, Input, Row, Select, Switch, Typography, Upload, Skeleton, theme, Spin } from "antd";
...
export default function AddNotification() {
    ...
    const onFinish = (value) => {
        ...
        try {
            ...
            value?.notification_category_id ? formData.append("notification_category_id", value?.notification_category_id) : "";
    ...
    const { mutate, isLoading } = useMutation(
        ...
        (data) => apiService.createNotification(data.load, true),
        {
   ...
   */
    ...
    const { mutate: fetchNotificationCategory, isLoading: loadingNotificationCategory } = useMutation(
        ...
        (payload) => apiService.getCategoryNotification(payload),
        {
            ...
            onSuccess: (data) => {
                if (data.success) {
                    const notificationCategories = data?.data?.data.map(item => ({ value: item?.notification_category_id, label: item?.notification_category_name })) || [];
    ...
    return actionsPermissionValidator(window.location.pathname, PermissionAction.ADD) ? (
        <>
            ...
            <Typography.Title level={5}>Add New Notification</Typography.Title>
            <Form name="form_item_path" form={form} layout="vertical" onFinish={onFinish}>
                <Row gutter={24}>
                    ...
                    <Col span={24}>
                        <Form.Item name="notification_message" label="Message"
                            rules={[
                        ...
                        >
                            <TextArea rows={4} placeholder="Enter Message Here" />
                        </Form.Item>
                    </Col>
                        ...
                        <div className="fixed_size" style={StyleSheet.uploadBoxStyle}>
                            ...
                            >
                                <Upload
                                    ...
                                    {notificationImage ? (
                                        <img src={notificationImage} alt="Notification Image" style={categoryStyle} />
                                    ) : (
                                        uploadButton
                                    )}
                                </Upload>

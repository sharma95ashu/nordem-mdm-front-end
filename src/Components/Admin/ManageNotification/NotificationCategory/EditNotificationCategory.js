...
export default function EditNotificationCategory() {
    ...
    const { mutate, isLoading } = useMutation(
        ...
        (data) => apiService.updateNotificationCategory(data.load, params.id),
        {
    ...
    const { refetch } = useQuery(
        ...
        () => apiService.getRequest(`/notification_category_list/${params.id}`),
        {
    ...
    useEffect(() => {
        ...
        setBreadCrumb({
            ...
            subtitle: "Edit Notification Category",
            path: Paths.users
        });
    }, []);
    return actionsPermissionValidator(window.location.pathname, PermissionAction.EDIT) ? (
        <>
            ...
            <Typography.Title level={5}>Edit Category</Typography.Title>

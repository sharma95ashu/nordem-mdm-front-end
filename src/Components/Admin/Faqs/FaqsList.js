...
const Faqs = () => {
  ...
  const fetchTableData = async (reset = false) => {
    try {
      ...
      let baseUrl = `/faqs/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  const { data, mutate: refetch, isLoading } = useMutation("fetchAllFaqs", fetchTableData);
          ...
          {actionsPermissionValidator(window.location.pathname, PermissionAction.EDIT) && (
            <>
              <Button
                type="default"
                primary
                onClick={() => navigate(`/${Paths.faqsEdit}/${record.faqs_id}`)}>
                Edit
              </Button>
            </>
          )}
  ...
  // api  to delete faqs data
  const { mutate: deleteMutate } = useMutation((data) => apiService.deleteSingleFaqs(data.load), {
  ...
  const handleDelete = (data) => {
    try {
      ...
      const body = {
        ids: Array.isArray(data) ? filterDeleteIds(data) : [data.faqs_id]
      };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All FAQs</Typography.Title>

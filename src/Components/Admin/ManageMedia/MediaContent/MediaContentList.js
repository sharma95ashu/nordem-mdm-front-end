...
const MediaCategoryList = () => {
  ...
  // Function to fetch medai content list data
  const fetchTableData = async (reset = false) => {
    try {
      // Define the base URL for the API endpoint
      let baseUrl = `/media_content/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  const {
    ...
    mutate: refetch,
    isLoading
  } = useMutation("fetchAllMediaContentListData", fetchTableData);
  ...
  const columns = [
    {
      title: "Media Title",
      width: "140px",
      dataIndex: "media_content_title",
      key: "media_content_title",
      sorter: (a, b) => a.media_content_title.localeCompare(b.media_content_title),
      render: (value, record) => (
        <Space>
    ...
    {
      title: "Media Sub Category Name",
      width: "140px",
      dataIndex: "child_category_name",
      key: "child_category_name",
      render: (value, record) => (
        <Space>
          <Typography.Text>{value || "-"}</Typography.Text>
        </Space>
...

    {
      ...
      render: (value) => (
        <Space>
          <img src={getFullImageUrl(value)} style={{ width: "120px", objectFit: "contain" }} />
        </Space>
      )
    },
    {
      ...
      render: (value, record) => (
        <Space>
          <ReactPlayer
            url={record?.media_upload_type == "url" ? record?.media_url : getFullImageUrl(value)}
            width="100%"
            height="100%"
            controls
          />
          ...
          {actionsPermissionValidator(window.location.pathname, PermissionAction.EDIT) && (
            <>
              <Button
                type="default"
                primary
                onClick={() => navigate(`/${Paths.mediaContentEdit}/${record.media_content_id}`)}>
                Edit
              </Button>
            </>
          )}
  ...
  // Function to delete media content
  const { mutate: deleteMutate, isLoading: deleteLoading } = useMutation(
    (data) => apiService.deleteMediaContent(data.load),
    {
  ...
  // Function to handle deletion of media content
  const handleDelete = (data) => {
    try {
      // Create a request body with media content array
      const body = {
        contentIds: [data.media_content_id]
      };

      // Define the request object with the API endpoint and request body
      let obj = { load: body };

      // Perform the API call for media content deletion
      deleteMutate(obj);
    } catch (error) {
      console.log(error);
    }
  ...
  useEffect(() => {
    setBreadCrumb({
      title: "Manage Media / Media Content",
      icon: "menuManagement",
      path: Paths.users
    });
  }, []);
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Media Content</Typography.Title>
      <Row gutter={[0, 6]}>
            ...
            <Flex justify="space-between" gap="middle">
              {actionsPermissionValidator(window.location.pathname, PermissionAction.ADD) && (
                <Flex style={StyleSheet.flexFullStyle} gap="middle" justify="flex-end">
                  <NavLink to={`/${Paths.mediaContentAdd}`}>
        ...
        <Col className="gutter-row" span={24}>
          <Content style={StyleSheet.contentSubStyle}>
            <Row>
              <Col flex="auto">
                <Search
                  ...
                  size="large"
                  placeholder="Search media within selected media category..."
                  enterButton="Search"
                  value={searchTerm}
            ...
            <Row className="noteStyle">
              Note: Use search media category to get all media related to the selected media
              category.
            </Row>
          </Content>
        </Col>

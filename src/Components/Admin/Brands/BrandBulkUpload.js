...
const BrandBulkUpload = (props) => {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (isEmpty(d["Parent"])) {
            ...
            if (!d["Brand name"] || isEmpty(d["Brand name"].toString())) {
              isValid = false;
              d["errors"]["Brand name"] = "Brand name cannot be empty";
            }
            ...
            if (
              ...
              isValid = false;
              d["errors"]["Status"] =
                "Brand Status cannot be empty and only active/inactive value is allowed";
            }

            if (
              !d["Brand Logo Image"] ||
              isEmpty(d["Brand Logo Image"].toString()) ||
              !URL_VALIDATION_REGEX.test(d["Brand Logo Image"])
            ) {
              isValid = false;
              d["errors"]["Brand Logo Image"] = "Brand Logo Image cannot be empty or is invalid";
            }

            if (d["Banner Image"] && !URL_VALIDATION_REGEX.test(d["Banner Image"])) {
              isValid = false;
              d["errors"]["Banner Image"] = "Banner Image is invalid";
            }
  ...
  const downloadSheet = () => {
    try {
      const data = brandSheetData.brandSheet;
      ...
      const instructionData = [
        { "Column Name": "Brand Name", Description: "This is brand Name" },
        { "Column Name": "Description", Description: "This is description" },
        { "Column Name": "Brand Logo Image", Description: "This is brand logo image link" },
        { "Column Name": "Banner Image", Description: "This is banner link" },
        { "Column Name": "Banner Mobile Image", Description: "This is banner image link" },
        { "Column Name": "Status", Description: "This is brand's status" }
      ];
      const ws3 = XLSX.utils.json_to_sheet(instructionData);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, ws, "Brand List");
      XLSX.utils.book_append_sheet(workbook, ws3, "Instructions");
      XLSX.writeFile(workbook, "Brand-Sample-Sheet.xlsx");
    } catch (error) {}
  };
  ...
  const { mutate, isLoading } = useMutation(
    ...
    (data) => apiService.uploadBrandsBulkUploadFile(data.load, data.randomId, true),
    {

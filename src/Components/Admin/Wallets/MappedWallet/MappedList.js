...
import MappedBulkUpload from "./MappedBulkUpload";
import MappedProgressCount from "./MappedProgressCount";
...
export default function MappedList() {
  ...
  const totalAmount = viewModalData?.reference.reduce((sum, item) => sum + item.amount, 0);
  ...
  const columns = [
    ...
    {
      title: "Used Balance Amount",
      width: checkInnerWidth() ? "120px" : "auto",
      dataIndex: "amount_redeemed",
      sorter: (a, b) => a.amount_redeemed - b.amount_redeemed,
      key: "amount_redeemed"
    },
    {
      title: "Balance Expiry Date",
      width: checkInnerWidth() ? "120px" : "180px",
    ...
    {
      title: "Status",
      dataIndex: "wallet_map_status",
      width: "120px",
      key: "wallet_map_status",
      sorter: (a, b) => a.wallet_map_status.localeCompare(b.wallet_map_status),
      render: (value) => (
        <>
          {value === "expired" ? (
    ...
    columns.push({
      ...
      render: (text, record) => (
        <Flex gap="small" justify="center" align="center">
          <Button type="default" primary onClick={() => showViewModal(record)}>
            View
          </Button>
          {actionsPermissionValidator(window.location.pathname, PermissionAction.EDIT) && (
            <>
  ...
  const viewColumns = [
    ...
    },
    {
      ...
      key: "wallet_name",
      render: (value, record) => {
        let wallet_name = record?.wallet?.wallet_name || viewModalData?.wallet_name || "N/A";
        return wallet_name;
      }
    },
    {
    ...
    },
    {
      ...
      sorter: (a, b) => a.adjustment_date.localeCompare(getAntDateTimeFormat(b.adjustment_date)),
      render: (value) => getAntDateTimeFormat(value),
      key: "adjustment_date"
    }
  ];

  // Function to fetch mapped wallet data
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/walletmaps/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  const { data, mutate: refetch, isLoading } = useMutation("fetchWalletMapData", fetchTableData);

  const filterDeleteIds = (deleteArr) => {
    try {
      const filteredKeys = data
        .filter((obj) => deleteArr.includes(obj.key))
        .map((obj) => obj.wallet_user_mapping_id);
      return filteredKeys;
    } catch (error) {}
  };

  // Function to handle deletion of mapped list
  const handleDelete = (data) => {
    try {
      // Create a request body with mapped wallet Ids array
      const body = {
        walletMapIds: Array.isArray(data) ? filterDeleteIds(data) : [data.wallet_user_mapping_id]
      };

      // Define the request object with the API endpoint and request body
      let obj = { load: body };

      // Perform the API call for mapped wallet deletion

      deleteMutate(obj);
    } catch (error) {}
  };

  // Function to handle Edit of mapped wallet
  const handleEdit = (data) => {
    navigate(`/${Paths.mappedEdit}/${data.wallet_user_mapping_id}`);
  };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>AB Wallets</Typography.Title>
      ...
      <Modal
        ...
        <Table
          columns={viewColumns}
          dataSource={viewModalData?.reference}
          pagination={false}
          loading={isLoading}
          bordered={true}
          scroll={{
          ...
          footer={() => (
            <>
              <Text>
                Available Balance: <b>₹{viewModalData.amount - totalAmount}</b>
                {totalAmount > 0 && (
                  <>
                    <br /> Used Balance: <b>₹{totalAmount}</b>
                  </>
                )}
              </Text>
            </>
      ...
      {showBulkUploadModel && (
        <MappedBulkUpload
          recallMappedApi={recallMappedApi}
          type={"product"}
        ...
        <Modal
          ...
          <>
            <Flex justify="center" align="middle" style={{ height: "100%" }}>
              <MappedProgressCount setPercentageCountLoading={setPercentageCountLoading} />
            </Flex>
          </>
        </Modal>
      ) : (

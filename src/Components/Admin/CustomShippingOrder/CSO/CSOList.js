...
export default function CSOList() {
  const { Search } = Input;
  ...
  const [displaySequenceData, setDisplaySequenceData] = useState([]);
  ...
  const handleSequence = (value) => {
    try {
      let parsedData = value.map(JSON.parse);
      const timelineData = parsedData?.map((item, index) => {
        return {
          children: item?.depotId ? item?.depotId?.split("-")[1] : "Head Office"
        };
      });
      setDisplaySequenceData(timelineData);
      showModal();
    } catch (error) {}
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value) => (
        <Flex
          ...
          <Typography.Text underline className="cursorPointer">
            View Sequence
          </Typography.Text>
        </Flex>
      )
    ...
    {
      title: "Status",
      dataIndex: "cso_status",
      width: "120px",
      key: "cso_status",
      sorter: (a, b) => a.cso_status.localeCompare(b.cso_status),
      render: (value) => (
        <>
          {value === "active" ? (
  ...
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/csos/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  const { mutate: deleteMutate } = useMutation(
    ...
    (data) => apiService.deleteCSO(data.load),
    {
      // Configuration options for the mutation
      onSuccess: (res, payload) => {
        if ((res, payload)) {
          if (data.length == payload?.load?.csoIds?.length) {
            setCurrent(1);
          }
  ...
  const { data, mutate: refetch, isLoading } = useMutation("fetchCSOData", fetchTableData);
  ...
  const handleDelete = (data) => {
    try {
      ...
      const body = {
        csoIds: Array.isArray(data) ? filterDeleteIds(data) : [data.cso_id]
      };
  ...
  const handleEdit = (data) => {
    navigate(`/${Paths.shippingSequencesEdit}/${data.cso_id}`);
  };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Shipping Sequences</Typography.Title>
            ...
            <Flex justify="space-between" gap="middle">
              ...
              <Flex justify="space-between" gap="middle">
                ...
                {actionsPermissionValidator(window.location.pathname, PermissionAction.ADD) && (
                  <NavLink to={`/${Paths.shippingSequencesAdd}`}>
      ...
      <>
        <Modal
          ...
          <Timeline items={displaySequenceData} />
        </Modal>
      </>
    </>
  ) : (

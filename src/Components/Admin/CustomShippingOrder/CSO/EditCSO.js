...
export default function EditCSO() {
  ...
  const { refetch: getSingleCSO } = useQuery(
    "fetchSingleCSO",
    () => apiService.getSingleCSODetail(params?.id),
    {
      enabled: false, // Enable the query by default
      onSuccess: (data) => {
        try {
          if (data) {
            form.setFieldValue("cso_name", data?.data?.cso_name);
            form.setFieldValue("cso_status", data?.data?.cso_status == "active" ? true : false);
            const parsedData = data?.data?.cso_sequence.map(JSON.parse);
            form.setFieldValue("cso_sequence", parsedData);
          }
        } catch (error) {}
      },
      onError: (error) => {
  ...
  const { mutate: updateCSO, isLoading } = useMutation(
    "updateSingleCSODetail",
    (payload) => apiService.updateSingleCSODetail(params?.id, payload),
    {
      enabled: false, // Enable the query by default
      onSuccess: (data) => {
        try {
  ...
  useEffect(() => {
    ...
    setBreadCrumb({
      ...
      subtitle: "Edit Shipping Sequence",
      path: Paths.users
    });
  }, []);
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.EDIT) ? (
    <>
      <Typography.Title level={5}>Edit Shipping Sequence</Typography.Title>

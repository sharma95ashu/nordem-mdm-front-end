...
const ComplianceDocuments = () => {
  ...
  const fetchTableData = async (reset = false) => {
    try {
      ...
      let baseUrl = `/compliance_documents/all/${
          ...
          {actionsPermissionValidator(window.location.pathname, PermissionAction.EDIT) && (
            <>
              <Button
                ...
                primary
                onClick={() =>
                  navigate(`/${Paths.complianceDocumentsEdit}/${record.compliance_id}`)
                }>
                Edit
              </Button>
            </>
  ...
  // api  to delete Compliance Documents
  ...
  const filterDeleteIds = (deleteArr) => {
    try {
      const filteredKeys = data
        ...
        .map((obj) => obj.compliance_document_id);
      return filteredKeys;
    } catch (error) {}
  };
  ...
  const handleDelete = (data) => {
    try {
      ...
      const body = {
        messageIds: Array.isArray(data) ? filterDeleteIds(data) : [data.compliance_document_id]
      };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Compliance Documents</Typography.Title>
            ...
            <Flex justify="space-between" gap="middle">
              ...
              {actionsPermissionValidator(window.location.pathname, PermissionAction.ADD) && (
                <Flex style={StyleSheet.flexFullStyle} gap="middle" justify="flex-end">
                  <NavLink to={`/${Paths.complianceDocumentsAdd}`}>

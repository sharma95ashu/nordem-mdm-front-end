...
export default function TagList() {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "150px",
      dataIndex: "show_in_category",
      key: "show_in_category",
    ...
    },
    {
      ...
      key: "tag_status",
      sorter: (a, b) => a.tag_status.localeCompare(b.tag_status),
      render: (value) => (
        <>
          {value === "active" ? (
  ...
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/tags/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  const { mutate: deleteMutate } = useMutation(
    ...
    (data) => apiService.deleteTag(data.load),
    {
      // Configuration options for the mutation
      onSuccess: (res, payload) => {
        if (data.length == payload?.load?.tagIds?.length) {
          setCurrent(1);
        }
  ...
  const handleDelete = (data) => {
    try {
      ...
      const body = {
        tagIds: Array.isArray(data) ? filterDeleteIds(data) : [data.tag_id]
      };
  ...
  const handleEdit = (data) => {
    navigate(`/${Paths.editTag}/${data.tag_id}`);
  };
  ...
  const downloadSheet = () => {
    try {
      const capitalizedData = data.map((obj) => {
        const newObj = {};
        for (let key in obj) {
          if (["tag_name", "tag_desc", "tag_status"].includes(key)) {
            if (key === "tag_desc") {
              newObj["Description"] = extractPlainText(obj[key]);
            } else if (key == "tag_status") {
      ...
      XLSX.utils.book_append_sheet(workbook, ws, "Tag List");
      XLSX.writeFile(workbook, "tag-sheet.xlsx");
    } catch (error) {}
  };

  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Tags</Typography.Title>

...
const ExecutivesList = () => {
  ...
  const handleEdit = (data) => {
    navigate(`/${Paths.executiveEdit}/${data.user_id}`);
  };
  const fetchTableData = async () => {
    try {
      setLoader(true);
      // Define the base URL for the API endpoint
      let baseUrl = `/executives/get-all-executives/`;
      const payload = {
      ...
      const data = await apiService.getExecutiveListForManageExecutives(baseUrl, payload);
      setLoader(false);
  ...
  const { mutate: deleteUser, isLoading: deleteLoading } = useMutation(
    ...
    (data) => apiService.deleteExecutive(data),
    {
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>Manage Executives</Typography.Title>

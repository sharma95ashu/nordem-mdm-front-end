...
export default function MarketingPlanLanguageList() {
  const { Search } = Input;
  ...
  const columns = [
    {
      title: "Language Name",
      dataIndex: "mp_language_name",
      sorter: (a, b) => a.mp_language_name.localeCompare(b.mp_language_name),
      key: "language_name"
    },
    {
      title: "Language Code",
      dataIndex: "mp_language_code",
      sorter: (a, b) => a.mp_language_code.localeCompare(b.mp_language_code),
      key: "mp_language_code"
    },
    {
      title: "Status",
      dataIndex: "status",
  ...
  // Function to fetch marketing plan language data
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/marketing_plan_language/all/${
  ...
  // Function to delete marketing plan language
  const { mutate: deleteMutate } = useMutation(
    (data) => apiService.deleteMarketingPlanLanguage(data.load),
    {
      // Configuration options for the mutation
      onSuccess: (res, payload) => {
        if (res) {
          if (data.length == payload?.load?.marketingPlanLanguageIds?.length) {
            setCurrent(1);
          }
  ...
  const { data, refetch, isLoading } = useQuery("fetchMarketingPlanLanguagesData", fetchTableData);

  const filterDeleteIds = (deleteArr) => {
    try {
      const filteredKeys = data
        .filter((obj) => deleteArr.includes(obj.key))
        .map((obj) => obj.language_id);
      return filteredKeys;
    } catch (error) {}
  };

  // Function to handle deletion of marketing plan language
  const handleDelete = (data) => {
    try {
      // Create a request body with marketing planLanguageIds array
      const body = {
        marketingPlanLanguageIds: Array.isArray(data) ? filterDeleteIds(data) : [data.language_id]
      };

      // Define the request object with the API endpoint and request body
      let obj = { load: body };

      // Perform the API call for language deletion
      deleteMutate(obj);
    } catch (error) {}
  };

  // Function to handle Edit of marketing plan language
  const handleEdit = (data) => {
    navigate(`/${Paths.marketingPlanLanguageEdit}/${data.language_id}`);
  };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Alert
        ...
        type="warning"
        message={
          "This section allows you to select a language that will be applied while adding products in the Products section."
        }
      />

      <Typography.Title className="pt-16" level={5}>
        All Languages
      </Typography.Title>
            ...
            <Flex justify="space-between" gap="middle">
              ...
              <Flex justify="space-between" gap="middle">
                ...
                {actionsPermissionValidator(window.location.pathname, PermissionAction.ADD) && (
                  <NavLink to={`/${Paths.marketingPlanLanguageAdd}`}>

...
// Marketing Plan List component
export default function MarketingPlanList() {
  const { Search } = Input;
  ...
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/marketing_plan/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  // Function to delete marketing plan
  const { mutate: deleteMutate } = useMutation(
    (data) => apiService.deleteMarketingPlan(data.load),
    {
      // Configuration options for the mutation
      onSuccess: (res, payload) => {
        if (res) {
          if (data.length == payload?.load?.marketingPlanIds?.length) {
            setCurrent(1);
          }
  ...
  const { data, refetch, isLoading } = useQuery("fetchMarketingPlanData", fetchTableData);

  const filterDeleteIds = (deleteArr) => {
    try {
      const filteredKeys = data
        .filter((obj) => deleteArr.includes(obj.key))
        .map((obj) => obj.marketing_plan_id);
      return filteredKeys;
    } catch (error) {}
  };

  // Function to handle deletion of marketing plan
  const handleDelete = (data) => {
    try {
      // Create a request body with marketingPlanIds array
      const body = {
        marketingPlanIds: Array.isArray(data) ? filterDeleteIds(data) : [data.marketing_plan_id]
      };
  ...
  const handleEdit = (data) => {
    navigate(`/${Paths.marketingPlanEdit}/${data.marketing_plan_id}`);
  };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Marketing Plans</Typography.Title>
            ...
            <Flex justify="space-between" gap="middle">
              ...
              <Flex justify="space-between" gap="middle">
                ...
                {actionsPermissionValidator(window.location.pathname, PermissionAction.ADD) && (
                  <NavLink to={`/${Paths.marketingPlanAdd}`}>

...
export default function NoticesList() {
  ...
  const columns = [
    {
      ...
      sorter: (a, b) => a.notice_text.localeCompare(b.notice_text),
      key: "notice_text",
      render: (value) => {
    ...
    },
    {
      title: "Show On",
      dataIndex: "show_on",
      sorter: (a, b) => a.show_on.localeCompare(b.show_on),
      key: "show_on",
      render: (value) => {
        return (
          <Space>
  ...
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/notice/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  const { mutate: deleteMutate } = useMutation((data) => apiService.deleteNotice(data.load), {
    // Configuration options for the mutation
    onSuccess: (res, payload) => {
      if (res) {
        if (data.length == payload?.load?.noticeIds?.length) {
          setCurrent(1);
        }
  ...
  const { data, refetch, isLoading } = useQuery("fetchNotices", fetchTableData);

  const filterDeleteIds = (deleteArr) => {
    try {
      const filteredKeys = data
        .filter((obj) => deleteArr.includes(obj.key))
        .map((obj) => obj.notice_id);
      return filteredKeys;
    } catch (error) {}
  };

  // Function to handle deletion of notice
  const handleDelete = (data) => {
    try {
      const body = {
        noticeIds: Array.isArray(data) ? filterDeleteIds(data) : [data.notice_id]
      };
      let obj = { load: body };
      deleteMutate(obj); // Perform the API call for notice deletion
    } catch (error) {}
  };

  // Function to handle Edit of notice
  const handleEdit = (data) => {
    navigate(`/${Paths.noticeEdit}/${data.notice_id}`);
  };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Notifications</Typography.Title>

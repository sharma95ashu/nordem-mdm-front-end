...
// Size Chart List component
export default function SizeChartsList() {
  const { Search } = Input;
  ...
  const fetchTableData = async (storeFilterData) => {
    try {
      ...
      let baseUrl = `/size_chart/all/${searchEnable.current ? 0 : current - 1}/${pageSize}`;
  ...
  const { mutate: deleteMutate } = useMutation((data) => apiService.deleteSizeChart(data.load), {
    // Configuration options for the mutation
    onSuccess: (res, payload) => {
      if (res) {
        if (data.length == payload?.load?.sizeChartIds?.length) {
          setCurrent(1);
        }
  ...
  const handleDelete = (data) => {
    try {
      ...
      const body = {
        sizeChartIds: Array.isArray(data) ? filterDeleteIds(data) : [data.size_chart_id]
      };
  ...
  const handleEdit = (data) => {
    navigate(`/${Paths.sizeChartEdit}/${data.size_chart_id}`);
  };
  ...
  return actionsPermissionValidator(window.location.pathname, PermissionAction.VIEW) ? (
    <>
      <Typography.Title level={5}>All Size Charts</Typography.Title>
